...
export default function Profile() {
  ...
  return (
    <Container className="mt-4" style={{ maxWidth: 980 }}>
      <Card className="mb-3 shadow-sm">
        <Card.Body className="d-flex justify-content-between align-items-center flex-column flex-md-row gap-3">
          ...
          <div className="text-md-end">
            <Button variant="outline-secondary" onClick={() => navigate("/profile/edit")}>ویرایش اطلاعات</Button>
          </div>
        </Card.Body>
      </Card>
      ...
      {orders.length === 0 ? (
        <Alert variant="info">شما هنوز سفارشی ثبت نکرده‌اید.</Alert>
      ) : (
        orders.map((order) => (
              ...
              <div className="d-flex justify-content-between align-items-start flex-column flex-md-row">
                <div>
                  ...
                  <div className="text-muted" style={{ fontSize: 13 }}>
                    تاریخ: {new Date(order.date).toLocaleString()}
                  </div>
                </div>
                  ...
                  {order.items.map((it) => {
                    ...
                    return (
                      <tr key={it.id}>
                        ...
                        <td>{formatToman(toToman(itemTotal))}</td>
                      </tr>
                    );
                  })}
                </tbody>
